...
// Mock del authMiddleware para incluir datos de contexto
const createMockAuthMiddleware = (contextOverrides: any = {}) => {
  return new Elysia()
    .derive(() => ({
      user: mockAuthUser,
...
// Mock global function getUserMaxLevel
const originalGetUserMaxLevel = global.getUserMaxLevel;
beforeAll(() => {
  global.getUserMaxLevel = mockGetUserMaxLevel;
});

afterAll(() => {
  global.getUserMaxLevel = originalGetUserMaxLevel;
});
...
describe('🔐 RBAC Middleware Tests - Fixed', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  describe('requirePermission Middleware', () => {
    const createTestApp = (contextOverrides: any = {}) => {
      const testContext = {
        user: mockAuthUser,
        isAuthenticated: true,
        clientIp: '192.168.1.100',
    ...
    it('should deny access for unauthenticated user', async () => {
      const app = createTestApp({ isAuthenticated: false, user: null });
      const response = await app.handle(new Request('http://localhost/test'));
  ...
  describe('Predefined Role Middlewares', () => {
    ...
    it('requireModeratorAccess should accept MODERATOR and above', async () => {
      const app = new Elysia()
        .derive(() => ({
          user: { ...mockAuthUser, role: 'MODERATOR' },
          isAuthenticated: true,
          clientIp: '192.168.1.100',
          userAgent: 'test-agent/1.0'
        }))
  ...
  describe('Resource Ownership Checks', () => {
    it('should correctly identify user as owner of their own profile', async () => {
      ...
      const response = await app.handle(new Request('http://localhost/users/user-123', {
        method: 'PUT'
      }));
    ...
    it('should check donation ownership correctly', async () => {
      ...
      const app = new Elysia()
        .derive(() => ({
          ...
          clientIp: '192.168.1.100',
          userAgent: 'test-agent/1.0',
          params: { id: 'donation-123' },
          path: '/donations/donation-123'
        }))
      ...
      const response = await app.handle(new Request('http://localhost/donations/donation-123'));
